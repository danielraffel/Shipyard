...
pub(super) fn reconcile_orphan_command<W: Write>(
    ...
    let related_processes = related_process_inventory(job_id, &envelope.cwd)?;
    ...
    let merge_cancellations =
        observer.observe_running(std::slice::from_ref(&job), &request_store, global_dir, None);
    let merged = merge_cancellations
        .into_iter()
        .find(|item| item.job_id == job_id && item.head_sha == job.sha)
        .ok_or_else(|| {
            CliFailure::new(
                1,
    ...
    if !related_process_inventory(job_id, &envelope.cwd)?.is_empty() {
        return Err(CliFailure::new(
            1,
...
mod tests {
    use super::{
        ...
        OrphanReconciliationRecord, canonical_job_sha256, logs_command,
        orphan_record_matches_queue, write_log,
    };
    use crate::job::{CancellationCause, CancellationProof, Job, Priority, ValidationMode};
