...
Joined laptops can enqueue `shipyard run` work to the controller through the
authenticated/idempotent `rpc-enqueue` boundary. Public laptop `shipyard ship`
is not wired to controller enqueue yet.
