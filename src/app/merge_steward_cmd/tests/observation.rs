...
fn unassigned_dispatch_uses_attempt_creation_before_job_started_at_exists() {
    let temp = tempfile::tempdir().expect("temp");
    let actions = fake_gh(&temp, "echo unexpected exact-attempt read >&2; exit 2");
    let run = StewardRun {
        id: 77,
        workflow_id: 88,
        run_attempt: 1,
    ...
    assert_eq!(
        dispatch_queue_age_origin(&actions, "owner/repo", &run).unwrap(),
        run.created_at
    );
}
...
#[cfg(unix)]
#[test]
fn rerun_dispatch_uses_exact_attempt_clock_and_refuses_missing_clock() {
    let temp = tempfile::tempdir().expect("temp");
    let actions = fake_gh(
        &temp,
        r#"
case "$*" in
  *"repos/owner/repo/actions/runs/77/attempts/2"*)
...
"#,
    ...
    let mut run = StewardRun {
        ...
        event: "merge_group".to_owned(),
        pull_request_number: Some(42),
        created_at: "2026-08-31T18:00:00Z".to_owned(),
        jobs: Vec::new(),
    };
    assert_eq!(
        dispatch_queue_age_origin(&actions, "owner/repo", &run).unwrap(),
