...
pub(crate) struct ProviderDeliveryFenceV1 {
    pub(crate) ledger_incarnation_ref: String,
    pub(crate) dispatcher_epoch_ref: String,
...
impl ProviderDeliveryFenceV1 {
    ...
    pub(crate) fn expected_idempotency_key(&self) -> String {
        #[derive(Serialize)]
        struct Inputs<'a> {
            domain: &'static str,
            ledger_incarnation_ref: &'a str,
            dispatcher_epoch_ref: &'a str,
            wake_id: &'a str,
            claim_id: &'a str,
            work_item_id: &'a str,
            work_generation: u64,
        ...
        let bytes = serde_json::to_vec(&Inputs {
            ...
            ledger_incarnation_ref: &self.ledger_incarnation_ref,
            dispatcher_epoch_ref: &self.dispatcher_epoch_ref,
...
pub(crate) fn validate_config(
    ...
    validate_digest(&config.executable_sha256)?;
    if config.deadline_seconds == 0
        || config.deadline_seconds > MAX_DEADLINE_SECONDS
        || config.max_stdout_bytes == 0
        || config.max_stdout_bytes > MAX_CAPTURE_BYTES
        || config.max_stderr_bytes == 0
        || config.max_stderr_bytes > MAX_CAPTURE_BYTES
    {
        return Err(refusal("provider wrapper execution bounds are invalid"));
    }
    Ok(())
}
...
pub(crate) fn validate_request(
    ...
    if fence.work_generation == 0 || fence.owner_generation == 0 || fence.attempt == 0 {
        return Err(refusal(
            "delivery fence generations and attempt must be nonzero",
        ));
    }
    for value in [
        &fence.ledger_incarnation_ref,
        &fence.dispatcher_epoch_ref,
        &fence.wake_id,
        &fence.claim_id,
        &fence.work_item_id,
        &fence.route_ref,
...
fn run_provider_wrapper_unix(
    ...
    let deadline = Instant::now()
        .checked_add(Duration::from_secs(config.deadline_seconds))
        .ok_or_else(|| refusal("provider wrapper deadline is unsupported"))?;
    let Ok(mut process) = ProcessTree::spawn(&mut command) else {
        return Ok(uncertain("verified-wrapper-launch-outcome-unknown"));
    };
    let stdout = process
        .take_stdout()
        .ok_or_else(|| refusal("provider wrapper stdout pipe is unavailable"))?;
    let stderr = process
        .take_stderr()
        .ok_or_else(|| refusal("provider wrapper stderr pipe is unavailable"))?;
    ...
    let stdout_bytes = stdout_reader
        .join()
        .map_err(|_| refusal("provider wrapper stdout reader panicked"))?
        .map_err(|_| refusal("provider wrapper stdout capture is unreadable"))?;
    let _stderr_bytes = stderr_reader
        .join()
        .map_err(|_| refusal("provider wrapper stderr reader panicked"))?
        .map_err(|_| refusal("provider wrapper stderr capture is unreadable"))?;
    if stdout_exceeded.load(Ordering::Acquire) || stderr_exceeded.load(Ordering::Acquire) {
...
/// Revalidate a protected response before another durable subsystem accepts it.
pub(crate) fn validate_protected_response(
    request: &ProviderWrapperRequestV1,
    response: &ProtectedProviderResponseV1,
) -> Result<ProviderWrapperRunResult, ProviderWrapperRefusal> {
    if hex::encode(Sha256::digest(&response.canonical_bytes)) != response.response_digest {
        return Err(refusal("protected provider response digest mismatch"));
    }
    let mapped = map_response(request, &response.canonical_bytes)?;
    let receipt = match &mapped {
        ProviderWrapperRunResult::Delivered {
            response_receipt, ..
        }
        ...
        | ProviderWrapperRunResult::Rejected {
            response_receipt, ..
        } => Some(response_receipt),
        ProviderWrapperRunResult::Uncertain {
            response_receipt, ..
        } => response_receipt.as_ref(),
    };
    if receipt != Some(response) {
        return Err(refusal(
            "protected provider response is malformed or fence-mismatched",
        ));
    }
    Ok(mapped)
}
...
mod tests {
    ...
    fn request(operation: ProviderWrapperOperationV1) -> ProviderWrapperRequestV1 {
        let mut fence = ProviderDeliveryFenceV1 {
            ledger_incarnation_ref: "ledger-1".into(),
            dispatcher_epoch_ref: "dispatcher-1".into(),
            wake_id: "wake-1".into(),
    ...
    #[test]
    fn execution_policy_bounds_refuse_before_deadline_construction() {
        let request = request(ProviderWrapperOperationV1::Submit);
        let mut invalid = config(Path::new("/does/not/matter"), digest("unused"));
        invalid.deadline_seconds = u64::MAX;
        assert!(
            run_provider_wrapper(&invalid, &ProviderWrapperEnvironment::default(), &request)
                .is_err()
        );

        invalid.deadline_seconds = 1;
        invalid.max_stdout_bytes = MAX_CAPTURE_BYTES + 1;
        assert!(
            run_provider_wrapper(&invalid, &ProviderWrapperEnvironment::default(), &request)
                .is_err()
        );
    ...
    #[test]
    fn recovered_exact_claim_uses_same_provider_idempotency_key() {
        let first = request(ProviderWrapperOperationV1::Submit);
        let recovered = request(ProviderWrapperOperationV1::Reconcile);
        assert_eq!(
    ...
    #[cfg(unix)]
    #[test]
    fn timeout_reaps_setsid_descendant_with_bounded_pipe_readers() {
        let request = request(ProviderWrapperOperationV1::Submit);
        let (directory, path, sha) = wrapper_c(
