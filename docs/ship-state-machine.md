...
| `shipyard ship` schema-v3 selected execution | Protected-base build/test policy plus trusted machine-global activation | Replaces `build` and `test` atomically. `--resume-from test` cannot activate the combined transaction because it would skip the substituted build command; Shipyard preserves the original stages and records a fallback diagnostic instead. A missing canonical build stage follows the same full-preserving fallback. |
