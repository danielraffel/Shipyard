...
| **Cross-repo merge-on-green stewardship** | `shipyard runner steward --repo <owner/pulp> --repo <owner/forge> --repo <owner/vellum> [--json]` (dry-run by default; `--apply` re-enrolls only the current exact green head, preserves native queue order, exact-head merges private/free-plan repos, bounds transient reruns, and coalesces only provably duplicate or superseded queued runs; opt out with `shipyard:no-auto-merge`) |
