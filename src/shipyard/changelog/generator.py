...
class ChangelogConfig:
    ...
    title: str = "Changelog"
    # Multi-line HTML comment block emitted between the intro and the
    # first entry. ``None`` (default) omits the block entirely so the
    # rendered file carries no shipyard-identifying text — consumers
    # that want a custom note set the full ``<!-- ... -->`` string.
...
def load_changelog_config(config: Config) -> ChangelogConfig:
    ...
    raw_header = section.get("header_comment")
    header_comment: str | None = (
        str(raw_header) if isinstance(raw_header, str) and raw_header else None
    )
...
def render_changelog(entries: list[Entry], cfg: ChangelogConfig) -> str:
    ...
    if cfg.header_comment is not None:
        lines.extend(cfg.header_comment.splitlines())
        lines.append("")
    for e in entries:
