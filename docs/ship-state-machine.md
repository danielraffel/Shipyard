...
| `shipyard ship` schema-v3 selected execution | Protected-base build/test policy plus trusted machine-global activation | Replaces `build` and `test` atomically. When any changed-surface target is configured, `--resume-from test` is hard-refused before target observation, activation persistence, or substitution because it could skip producer builds and test stale warm artifacts; restart from `build` or start fresh. A missing canonical build stage follows the full-preserving fallback. |
...
- **App-authenticated branch cleanup:** `--delete-branch` preflights the machine-global trusted native Git path before any merge or queue mutation. Cleanup runs the exact-SHA deletion lease from a newly initialized isolated bare repository. A machine that has not configured `github.auth.privileged_git_binary` therefore fails explicitly before merge instead of silently leaving the head branch behind; ambient keyring auth retains its existing path.
