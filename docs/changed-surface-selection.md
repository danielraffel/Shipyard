...
`--resume-from test` path is hard-refused for schema v3, because resuming after
`build` must never turn its substituted no-op `test` stage—or stale warm
artifacts—into passing selected evidence. Restart from `build` or start a fresh
validation. The
