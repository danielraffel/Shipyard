        ...
        *,
        ...
        if args[:4] == ["gh", "release", "download", "--repo"]:
            ...
            if "--output" in args:
                ...
                output = Path(args[args.index("--dir") + 1]) / pattern
                output.write_bytes(pattern.encode())
            return ""
...
class ReleaseMacosLocalTests(unittest.TestCase):
    ...
    def test_provider_expected_from_first_paired_release(self) -> None:
        self.assertFalse(release_macos_local._provider_expected_for_tag("v0.126.2"))
        self.assertTrue(release_macos_local._provider_expected_for_tag("v0.126.3"))
        self.assertTrue(release_macos_local._provider_expected_for_tag("latest"))
