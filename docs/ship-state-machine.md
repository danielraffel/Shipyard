...
| `shipyard ship` schema-v3 selected execution | Protected-base build/test policy plus trusted machine-global activation | Replaces `build` and `test` atomically. `--resume-from test` is hard-refused because it would skip the substituted build command and could test stale warm artifacts; restart from `build` or start a fresh validation. A missing canonical build stage follows the full-preserving fallback. |
