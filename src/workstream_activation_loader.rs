...
use crate::workstream_continuation_config::{
    WorkstreamContinuationConfig, trusted_workstream_continuation_config_from_bytes,
};
...
impl WorkstreamActivationLoader {
    ...
    fn load_current(&self) -> Result<WorkstreamActivationState, WorkstreamActivationRefusal> {
        ...
        let Some(config_bytes) = config_before.as_deref() else {
        ...
        if trusted_workstream_continuation_config_from_bytes(config_bytes, "activation-probe")
        ...
        let policy = trusted_workstream_continuation_config_from_bytes(config_bytes, &machine_tag)
