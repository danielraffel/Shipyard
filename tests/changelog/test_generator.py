...
def test_header_comment_default_omits_comment_block(tmp_path: Path) -> None:
    repo = seed_repo(tmp_path)
    commit(repo, "a.txt", "a", "feat: a (#1)", date="2026-01-01T00:00:00+00:00")
    tag(repo, "v0.1.0", date="2026-01-01T00:00:00+00:00")
...
def test_header_comment_override_emitted_verbatim(tmp_path: Path) -> None:
    """Consumers preserving an existing CHANGELOG.md header set this field.

    Multi-line strings are split on newlines and each becomes its own
    rendered line, with one blank separator before the first entry.
    """
    repo = seed_repo(tmp_path)
    commit(repo, "a.txt", "a", "feat: a (#1)", date="2026-01-01T00:00:00+00:00")
    tag(repo, "v0.1.0", date="2026-01-01T00:00:00+00:00")
    ...
    cfg = _cfg(header_comment=custom)
    text = render_changelog(build_entries(cfg, cwd=repo), cfg)
    assert custom in text
    # blank line separator before first entry
    assert custom + "\n\n<a id=" in text
