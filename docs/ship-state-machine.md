...
|-----------------------------|-----------------------------------------------------|---------------------------------------------------------|
| `shipyard pr` prospective changed-surface push | Trusted machine-global `changed_surface_execution`; protected-base selector policy; clean local head/tree; configured repository-relative `core.hooksPath/pre-push` | Before PR discovery, prepares a private nonce-bound prospective receipt only for one non-delete branch update. The hook must be a protected-base-tracked executable regular file with byte-identical content before and after the supervised push. Shipyard marks push success in the parent process, then verifies the hook result against the exact head, tree, changed paths, selected tests, and hook digest. No `ShipState` pass evidence is written: missing or drifting identity merely declines the optimization so ordinary full validation remains authoritative. |
