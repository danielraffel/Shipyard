...
use super::{
    WorkLedger, WorkLedgerError, WorkLedgerResult, configure_durable, digest, opaque_ref,
    random_opaque_ref, validate_digest, verify_integrity, verify_supported_schema,
};
...
impl WorkLedger {
    ...
    pub(super) fn reconcile_protected_object_storage(&self) -> WorkLedgerResult<()> {
        self.reconcile_protected_object_storage_with_verification(false)
    }

    pub(super) fn reconcile_and_verify_protected_object_storage(&self) -> WorkLedgerResult<()> {
        self.reconcile_protected_object_storage_with_verification(true)
    }

    fn reconcile_protected_object_storage_with_verification(
        &self,
        verify_registered_objects: bool,
    ) -> WorkLedgerResult<()> {
        let parent = self
            .path
            .parent()
    ...
    pub(super) fn verify_protected_object_storage(
        ...
        let mut statement = connection.prepare(
            "SELECT object_ref, work_item_id, kind, profile_ref, storage_name,
               ...
               FROM protected_objects ORDER BY object_ref LIMIT 4097",
        )?;
        ...
        for (observed_storage_name, record) in rows {
            if observed_storage_name != storage_name(&record.object_ref)? {
                return Err(WorkLedgerError::Refused(
                    "protected object storage name does not match its object reference".to_owned(),
                ));
            }
            expected.insert(observed_storage_name, record);
        }
    ...
    pub(super) fn put_protected_object_with_writer_domain(
        ...
        let created_at = self.clock.observe(&transaction)?.timestamp.to_rfc3339();
        transaction.execute(
...
fn cleanup_failed_publication(
    ...
    bytes: &[u8],
    publication_error: &WorkLedgerError,
) -> WorkLedgerResult<()> {
    use rustix::fs::{AtFlags, unlinkat};

    let observed = read_object_from_directory(directory, name, record).map_err(|cleanup_error| {
        WorkLedgerError::Refused(format!(
            "protected object publication failed ({publication_error}) and exact cleanup verification failed ({cleanup_error})"
        ))
    })?;
    if observed != bytes {
        return Err(WorkLedgerError::Refused(format!(
            "protected object publication failed ({publication_error}) and cleanup bytes differ"
        )));
    }
    unlinkat(directory, name, AtFlags::empty()).map_err(|cleanup_error| {
        WorkLedgerError::Refused(format!(
            "protected object publication failed ({publication_error}) and exact cleanup failed ({cleanup_error})"
        ))
    })?;
    directory.sync_all().map_err(|cleanup_error| {
        WorkLedgerError::Refused(format!(
            "protected object publication failed ({publication_error}) and cleanup sync failed ({cleanup_error})"
        ))
    })?;
    Ok(())
}
...
fn cleanup_failed_publication(
    ...
    Err(WorkLedgerError::Refused(format!(
        "protected object publication failed ({publication_error}) and exact cleanup is unavailable"
    )))
}
...
fn read_object_file(
    ...
    let bytes = read_object_from_directory(&directory, name, record)?;
    verify_directory_binding(parent, &directory)?;
    Ok(bytes)
}
...
fn write_object_to_directory(
    ...
    let pending_name = format!(".pending-{}", random_opaque_ref("object")?);
    let opened = openat(
        directory,
        &pending_name,
    ...
    match opened {
        Ok(file) => {
            ...
            if let Err(error) = result {
                if published {
                    cleanup_failed_publication(directory, name, record, bytes, &error)?;
                }
