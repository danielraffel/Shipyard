...
pub fn trusted_workstream_continuation_config(
    ...
    trusted_workstream_continuation_config_from_table(&trusted.data, observed_origin_machine)
}

/// Parse continuation policy from bytes already pinned by the activation
/// authority. This avoids reopening a mutable pathname after authentication.
pub(crate) fn trusted_workstream_continuation_config_from_bytes(
    bytes: &[u8],
    observed_origin_machine: &str,
) -> Result<Option<WorkstreamContinuationConfig>, WorkstreamContinuationConfigError> {
    let text = std::str::from_utf8(bytes)
        .map_err(|_| refusal("trusted machine policy is not UTF-8".to_owned()))?;
    let table = text
        .parse::<toml::Table>()
        .map_err(|error| refusal(format!("parse trusted machine policy: {error}")))?;
    trusted_workstream_continuation_config_from_table(&table, observed_origin_machine)
}

fn trusted_workstream_continuation_config_from_table(
    table: &toml::Table,
    observed_origin_machine: &str,
) -> Result<Option<WorkstreamContinuationConfig>, WorkstreamContinuationConfigError> {
    let Some(value) = table.get(POLICY_KEY) else {
        return Ok(None);
    };
