...
| **Supply non-secret machine paths to every fresh worktree** | Track exact `[project].repository` plus `validation.<name>.machine_environment = ["NAME"]`; put each host's value under machine-global `[repository_environment."OWNER/REPO"]`. Shipyard binds the case-sensitive GitHub origin, ignores repo/local attempts to supply values, rejects secret-bearing names, and snapshots resolved values under owner-protected queue state for session-independent execution. |
