...
| **Runner provisioning environment contract** | Registration uses one fleet-pinned Actions runner with auto-update disabled, a system-first `.path`, and runner-private `_toolcache/{rustup,cargo}` homes; never symlink toolchain homes to a shared/external build volume. Pinned upgrades stop and later restart the existing registered service without rerunning fresh configuration/install, and readiness probes must not leak child output. |
